...
module.exports = class PG extends EventEmitter {
    ...
    for (let i in rows) {
      ...
      for (let j in cols) {
        ...
        if (!columns.includes(columnName)) {
          await this.addColumn(table, columnName, row[columnName])
  ...
  async addColumn (table, columnName, exampleValue) {
    if (exampleValue === null) { return }
    if (exampleValue === undefined) { return }
    if (exampleValue === '') { return }

    let type = 'TEXT'
    if (typeof exampleValue === 'number' && exampleValue.toString().indexOf('.') < 0) {
      type = 'BIGINT'
    } else if (typeof exampleValue === 'number' && exampleValue.toString().indexOf('.') > 0) {
      type = 'FLOAT'
